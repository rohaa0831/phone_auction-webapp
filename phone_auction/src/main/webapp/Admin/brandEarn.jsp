...
out.println("Invalid or empty brand. Please try again.");
} else {
// Execute a query to get all closed auctions with the given modelNum
ResultSet rs = stmt.executeQuery("SELECT * FROM auction WHERE brand = '" + brand + "' AND endDate < CURDATE()");
...
if (summation == 0) {
// Print an error message
out.println("There are no closed auctions with the given brand.");
} else {
// Print the summation of the current prices
out.println("The summation of the current prices for auctions with brand '" + brand + "' is: " + summation);
}
}
%>
